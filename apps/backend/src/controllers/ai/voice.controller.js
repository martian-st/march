...
const chainOfThoughtService = new ChainOfThoughtService(environment.GOOGLE_AI_API_KEY);
const objectManagerService = new AdvancedObjectManagerService(environment.GOOGLE_AI_API_KEY);
const calendarService = new CalendarIntegrationService(environment.GOOGLE_AI_API_KEY);
...
export const processVoiceCommand = async (req, res) => {
    try {
        ...
        const voiceResult = await voiceService.processVoiceCommand(transcribedText, context);
        ...
        const assistantQuery = await voiceService.convertToAssistantQuery(voiceResult);
        ...
        switch (assistantQuery.type) {
            case 'create':
                assistantResult = await objectManagerService.createObject(assistantQuery.query, assistantQuery.context);
                break;
                
            case 'find':
                assistantResult = await objectManagerService.findObjects(assistantQuery.query, assistantQuery.options);
                break;
                
            case 'calendar':
                assistantResult = await calendarService.processCalendarRequest(assistantQuery.query, assistantQuery.action, assistantQuery.context);
                break;
                
            case 'process':
            default:
                assistantResult = await chainOfThoughtService.processComplexRequest(assistantQuery.query, assistantQuery.context);
                break;
        }

        // Step 4: Generate voice-friendly response
        const voiceResponse = voiceService.generateVoiceResponse(assistantResult, assistantQuery.voiceMetadata);
...
export const voiceHealthCheck = async (req, res) => {
    try {
        ...
        const testResult = await voiceService.processVoiceCommand('Hello, can you help me?', {});
