...
`;
            ...
            return simpleConversational.some(
                (word) => userPrompt.toLowerCase().trim() === word
            );
        }
    }
...
`;
            ...
            return {
                isConversational: true,
                response:
          "Hello! I'm March AI, your productivity assistant. I can help you with tasks, scheduling, and much more. What would you like me to help you with?",
                success: true
            };
        }
    }
   ...
   */
    async processComplexRequest (userPrompt, userId, context = {}) {
        ...
        this.detectFollowUpQuestion(userPrompt, conversationHistory)
            ) {
                return await this.handleFollowUpQuestion(
                    userPrompt,
                    userId,
                    conversationHistory
                );
            ...
            if (pendingClarification) {
                return await this.handleClarificationResponse(
                    userPrompt,
                    userId,
            ...
            if (
                this.needsClarification(analysis, userPrompt) &&
        this.isHighlyAmbiguous(userPrompt)
            ) {
            ...
            const reasoningChain = await this.generateReasoningChain(
                analysis,
                context
            );

            // Execute the reasoning chain
            const executionResult = await this.executeReasoningChain(
                reasoningChain,
                userId
            );
   ...
   */
    isHighlyAmbiguous (userPrompt) {
        ...
        return (
            extremelyVague.some((pattern) => pattern.test(prompt)) &&
   ...
   */
    calculateAmbiguityScore (analysis, userPrompt) {
        ...
        const pronounCount = words.filter((word) =>
            criticalPronouns.includes(word)
        ).length;
   ...
   */
    detectFollowUpQuestion (userPrompt, conversationHistory) {
        ...
        return (
            followUpIndicators.some((indicator) => lowerPrompt.includes(indicator)) ||
      lowerPrompt.split(" ").length < 6
        ...
        clarificationData.answers.forEach((answer, index) => {
            ...
            case "text":
                if (answer.answer.length > 0) {
                    ...
                    enhanced.actions[0].requiredInfo =
              enhanced.actions[0].requiredInfo?.filter(
                  (info) =>
                      !info.toLowerCase().includes("title") &&
                  !info.toLowerCase().includes("content")
              ) || [];
                }
                break;
            case "date":
   ...
   */
    getDefaultClarificationQuestions (originalPrompt, analysis) {
      ...
      originalPrompt.toLowerCase().includes("task")
        ) {
            questions.push({
                question: "What should the task title be?",
                type: "text",
        ...
        if (
            originalPrompt.toLowerCase().includes("meeting") ||
      originalPrompt.toLowerCase().includes("schedule")
        ) {
            questions.push({
   ...
   */
    async generateReasoningChain (analysis, context) {
        ...
        const aiResult = await aiErrorHandler.executeWithRetry(
            async () => {
                const result = await this.model.generateContent(chainPrompt);
                return result.response.text();
            },
            { method: "generateReasoningChain", analysis, context }
        );

        if (!aiResult.success) {
            console.error("AI reasoning chain failed:", aiResult.error);
            // Return a simple fallback chain
            return [
                {
                    step: 1,
                    reasoning:
            "I'm having trouble processing your request right now, but I'll try to help.",
                    method: "fallback",
                    parameters: { message: aiResult.error.message },
                    expectedOutcome: "Provide fallback response",
                    failureHandling: "Show error message",
                    outputForNext: "error_state"
                }
            ];
        }
    ...
    async executeReasoningChain (reasoningChain, userId) {
        ...
        for (const step of reasoningChain) {
            try {
                ...
                const fallbackResult = await this.handleStepFailure(
                    step,
                    error,
                    userId
                );
   ...
   */
    async executeUpdateStep (step, userId, contextData) {
        ...
        const object = await Object.findByIdAndUpdate(objectId, updateData, {
            new: true
        });
   ...
   */
    async handleStepFailure (step, error, userId) {
        console.log(
            `Handling failure for step ${step.stepNumber}: ${error.message}`
        );
        ...
        try {
            ...
            return {
                ...
                message: error.message,
                suggestion:
          "I encountered an issue completing this step. Please try again or rephrase your request.",
    ...
    async synthesizeFinalResult (results) {
        ...
        if (successfulSteps.length === 0) {
            return {
                success: false,
                message:
          "I wasn't able to complete your request. Please try rephrasing or breaking it into smaller parts.",
   ...
   */
    async generateConversationalResponse (resultData) {
        const { createdObjects, foundObjects, analyses, successfulSteps } =
      resultData;
        ...
        Successful Steps: ${JSON.stringify(
